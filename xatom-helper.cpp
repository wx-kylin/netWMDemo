...
void XAtomHelper::setTitlebarValue(int winId, int value)
{
    if (m_ukuiTitlebarValue == None)
        return;
    XChangeProperty(QX11Info::display(), winId, m_ukuiTitlebarValue, m_ukuiTitlebarValue,
                    32, XCB_PROP_MODE_REPLACE, (const unsigned char *)&value, 1);
}
...
XAtomHelper::XAtomHelper(QObject *parent) : QObject(parent)
{
    ...
    m_ukuiTitlebarValue = XInternAtom(QX11Info::display(), "_KWIN_UKUI_TITLEBAR", false);
}
